...
export default function ArchivePage() {
  ...
  return (
    <main>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '56px 40px 80px' }} className="animate-fade-up">
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 20, marginBottom: 32 }}>
          <h1 style={{ fontFamily: 'var(--font-playfair)', fontSize: 40, fontWeight: 800, letterSpacing: '-1px', color: 'var(--t1)' }}>Archive</h1>
          <div style={{ position: 'relative' }}>
            <input
              ...
              style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 8, padding: '10px 16px 10px 38px', fontSize: 14, color: 'var(--t1)', fontFamily: 'inherit', outline: 'none', width: 240, transition: 'border-color 0.18s' }}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 32, marginBottom: 64 }}>
          {visible.map((post) => (
            <Link key={post.slug} href={`/${post.slug}`} style={{ display: 'block', transition: 'opacity 0.2s' }} className="archive-card">
              <div style={{ aspectRatio: '16/9', background: 'var(--ph)', borderRadius: 8, marginBottom: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid var(--border)' }}>
